...
const sendEmail = async (options) => {
  try {
    // 🚨 DEBUG LOGS: Check if env variables are loaded properly
    console.log("===== DEBUGGING ENV VARIABLES =====");
    console.log("EMAIL_USER:", process.env.EMAIL_USER ? `'${process.env.EMAIL_USER}'` : "UNDEFINED ❌");
    console.log("EMAIL_PASS:", process.env.EMAIL_PASS ? `Loaded (Length: ${process.env.EMAIL_PASS.length}) -> Value: '${process.env.EMAIL_PASS}'` : "UNDEFINED ❌");
    console.log("===================================");
